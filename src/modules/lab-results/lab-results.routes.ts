...
export async function labResultsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{ Params: { patientId: string }; Querystring: { category?: string; reportType?: string } }>('/patient/:patientId', { preHandler: requireRole('admin_doctor', 'doctor') }, controller.getByPatient.bind(controller))
  ...
  fastify.put<{ Params: { id: string } }>('/:id', { preHandler: requireRole('admin_doctor', 'doctor') }, controller.update.bind(controller))
