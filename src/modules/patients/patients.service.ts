    ...
    const newPatient = await this.db.transaction(async (tx) => {
      const [insertedPatient] = await tx
        ...
        .values({
          ...
          birthDateExact: dto.patient.birth_date_exact ?? null,
    ...
    const patientData = await this.db.transaction(async (tx) => {
      const [patient] = await tx
        .select({
          ...
          birth_date_exact: patients.birthDateExact,
          phone: patients.phone,
    ...
    const result = await this.db.transaction(async (tx) => {
      ...
      const updatedPatientList = await tx
        ...
        .set({
          ...
          ...(p.birth_date_exact !== undefined && { birthDateExact: p.birth_date_exact ?? null }),
    ...
    const result = await this.db.transaction(async (tx) => {
      const [updated] = await tx
        ...
        .set({
          ...
          ...(data.birth_date_exact !== undefined && { birthDateExact: data.birth_date_exact ?? null }),
